...
def ema_gate(df, asset_price):
    ...
    assetATR = indicators.calculateATR(df)
    ...
    if asset_price > assetEMA['ema'] + assetATR['atr'] and asset_price > assetEMA['dema'] + assetATR['atr']:
        emaLong = True
...
def ichimoku_gate(df, asset_price):
    """
...
def trend_gate(df):
    ...
    trendShort = False
    # check if conditions for long trade are met
    if assetTrend['greaterCount'] * 3 >= assetTrend['lessCount']:
        trendLong = True
    # check if conditions for short trade are met
    if assetTrend['lessCount'] * 3 >= assetTrend['greaterCount']:
        trendShort = True
    # return results as a dictionary
    return {
        'long': trendLong, 
...
def bollinger_gate(df, asset_price):
    """
...
def cloud_gate(df, asset_price):
    ...
    cloudSort = False
    # Check if conditions for long trade are met
    if assetICHIMOKU['current']['cloud_size'] > (.1 * asset_price) / 100.0:
        cloudLong = True
        cloudSort = True
